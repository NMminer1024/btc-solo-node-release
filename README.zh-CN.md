...
- 自己验证比特币主网的每一个区块，不依赖任何中心化服务；
- 给矿机提供本地挖矿入口，挖到的区块奖励 100% 归你，没有手续费分成；
- 实时监控节点、矿机、设备状态（屏幕 + 网页）。

**不能做的（常见误区）：**

- **它不是钱包** —— 不保存你的私钥，不能帮你收币、转币、花币；
- **它自己不会挖矿** —— 设备本身没有算力，必须接入你自己的矿机才能挖；
- **独矿 ≠ 天天出块** —— 挖到区块完全靠运气，算力小时可能几个月都挖不到一个块，这是独矿的正常现象，不是故障；
- **它不是全量节点** —— 采用裁剪模式，只保留最近约 4 GB 的区块数据，不能提供完整的历史区块查询；
- **它不提供交易、兑换等服务** —— 只做节点验证和挖矿这一件事。
